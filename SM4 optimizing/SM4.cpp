...
void subkeyGen(int n);
void RoundFuncT(unsigned long x0, unsigned long x1, unsigned long x2, unsigned long x3, int n);
void SM4Encryption(int q, unsigned long input[]);
...
{
	unsigned long P[] = { 0x01234567, 0x89abcdef,0xfedcba89,0x76543210 };
    //P是4个输入的明文块
	SM4Encryption(0, P);
...
void SM4Encryption(int q, unsigned long input[])
{
	...
	for (int i = 1; i < 33; i++)
	{
		RoundFuncT(cipher[i - 1], cipher[i], cipher[i + 1], cipher[i + 2], i);
	}
